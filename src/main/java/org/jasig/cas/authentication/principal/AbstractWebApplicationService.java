...
public abstract class AbstractWebApplicationService implements SingleLogoutService {
    ...
    public boolean matches(final Service service) {
        try {
        	...
        	String internalId = this.id;
        	if (this.id.contains("?ticket=")){
        		internalId = id.substring(0, id.indexOf("?"));
        	}
        	
            final String thisUrl = URLDecoder.decode(internalId, "UTF-8");
            final String serviceUrl = URLDecoder.decode(service.getId(), "UTF-8");

            logger.info("interanlId {}", internalId);
            logger.info("id: {}", id);
            logger.info("originalId {}", originalUrl);
